...
void loop()
{
  ...
  GyX2=Wire.read()<<8|Wire.read();  //0x3B (GYRO_XOUT_H) & 0x3C (GYRO_XOUT_L)     
  GyY2=Wire.read()<<8|Wire.read();  //0x3D (GYRO_YOUT_H) & 0x3E (GYRO_YOUT_L)
  GyZ2=Wire.read()<<8|Wire.read();  //0x3F (GYRO_ZOUT_H) & 0x40 (GYRO_ZOUT_L)
  //Tmp=Wire.read()<<8|Wire.read();  //0x41 (TEMP_OUT_H) & 0x42 (TEMP_OUT_L)
  AcX2=Wire.read()<<8|Wire.read();  //0x43 (ACCEL_XOUT_H) & 0x44 (ACCEL_XOUT_L)
  AcY2=Wire.read()<<8|Wire.read();  //0x45 (ACCEL_YOUT_H) & 0x46 (ACCEL_YOUT_L)
  AcZ2=Wire.read()<<8|Wire.read();  //0x47 (ACCEL_ZOUT_H) & 0x48 (ACCEL_ZOUT_L)

  //Adapts to range from 0 to 2000 m/s^2
  AcX2 = map(AcX2,minVal,maxVal,2000,0);
  AcY2 = map(AcY2,minVal,maxVal,2000,0);
  AcZ2 = map(AcZ2,minVal,maxVal,2000,0);
  ...
  //Serial.print(" | Tmp = "); Serial.print(float(Tmp + offsetTempLevels)/340 + offsetTemp);
  //Send X axis gyroscope angle value for serial monitor
  Serial.print(" | GyX2 = "); Serial.print(xAng2);   
  //Send Y axis gyroscope angle value for serial monitor  
  Serial.print(" | GyY2 = "); Serial.print(yAng2);
  //Send Z axis gyroscope angle value for serial monitor
  Serial.print(" | GyZ2 = "); Serial.println(zAng2);
  //Wait 300 ms, after that, start new measurement and display.

   Serial.print(" roll in degree "); Serial.println(xAng2-xAng);
   Serial.print(" pitch in degree "); Serial.println(yAng2-yAng);
  Serial.print(" yaw in degree "); Serial.println(zAng2-zAng);
